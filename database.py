...
def create_meeting(title, video_link=""):
    ...
    while conn.execute("SELECT 1 FROM meetings WHERE code=?", (code,)).fetchone():
        code = _gen_code()
    conn.execute(
        "INSERT INTO meetings (code, title, video_link) VALUES (?,?,?)",
        (code, title, video_link)
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (code, "created", "Meeting '{}' created".format(title))
    )
    conn.commit()
    conn.close()
    return code
...
def get_meeting(code):
    conn = get_conn()
    row = conn.execute(
        "SELECT code, title, status, video_link, confirmed_date, confirmed_start, confirmed_end, created_at "
        "FROM meetings WHERE code=?",
        (code,)
    ).fetchone()
    conn.close()
    return row
...
def get_archived_meetings():
    conn = get_conn()
    rows = conn.execute(
        "SELECT code, title, status, created_at, '' FROM meetings "
...
def update_meeting_link(code, link):
    conn = get_conn()
    conn.execute(
        "UPDATE meetings SET video_link=?, updated_at=datetime('now') WHERE code=?",
        (link, code)
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (code, "link_updated", "Video link updated")
    )
    conn.commit()
    conn.close()
...
def confirm_meeting_slot(code, date, start, end):
    conn = get_conn()
    conn.execute(
        "UPDATE meetings SET status='confirmed', confirmed_date=?, confirmed_start=?, "
        "confirmed_end=?, updated_at=datetime('now') WHERE code=?",
        (str(date), str(start), str(end), code)
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (code, "confirmed", "Confirmed: {} {}-{}".format(date, start, end))
    )
    conn.commit()
    conn.close()
...
def cancel_meeting(code, reason=""):
    conn = get_conn()
    conn.execute(
        "UPDATE meetings SET status='cancelled', updated_at=datetime('now') WHERE code=?",
        (code,)
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (code, "cancelled", "Cancelled: {}".format(reason))
    )
    conn.commit()
    conn.close()
...
def archive_meeting(code):
    conn = get_conn()
    conn.execute(
        "UPDATE meetings SET status='archived', updated_at=datetime('now') WHERE code=?",
        (code,)
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
...
def restore_meeting(code):
    conn = get_conn()
    conn.execute(
        "UPDATE meetings SET status='active', confirmed_date='', confirmed_start='', "
        "confirmed_end='', updated_at=datetime('now') WHERE code=?",
        (code,)
    )
    conn.execute(
...
def postpone_meeting(code, reason=""):
    ...
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (code, "postponed", "Postponed: {}".format(reason))
    )
    conn.commit()
    conn.close()
...
def add_availability(meeting_code, name, email, role, date, start_time, end_time):
    ...
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (meeting_code, "availability_added",
         "{} ({}) — {} {}-{}".format(name, role, date, start_time, end_time))
    )
    conn.commit()
    conn.close()
...
def get_availability(meeting_code):
    ...
    rows = conn.execute(
        ...
        "FROM availability WHERE meeting_code=? ORDER BY date, start_time",
        (meeting_code,)
    ).fetchall()
    conn.close()
    return rows
...
def get_meeting_history(code):
    conn = get_conn()
    rows = conn.execute(
        "SELECT action, details, created_at FROM meeting_history "
        "WHERE meeting_code=? ORDER BY created_at DESC",
        (code,)
    ).fetchall()
    conn.close()
    return rows
...
def create_proposal(meeting_code, proposer_name, proposer_email, proposer_role,
                    proposed_date, proposed_start, proposed_end):
    conn = get_conn()
    conn.execute(
        "INSERT INTO proposals (meeting_code, proposer_name, proposer_email, proposer_role, "
        "proposed_date, proposed_start, proposed_end) VALUES (?,?,?,?,?,?,?)",
        (meeting_code, proposer_name, proposer_email, proposer_role,
         str(proposed_date), str(proposed_start), str(proposed_end))
    )
    conn.execute(
        "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
        (meeting_code, "proposal_created",
         "{} proposed {} {}-{}".format(proposer_name, proposed_date, proposed_start, proposed_end))
    )
    conn.commit()
    conn.close()
...
def get_proposals(meeting_code):
    conn = get_conn()
    rows = conn.execute(
        "SELECT id, proposer_name, proposer_email, proposer_role, proposed_date, "
        "proposed_start, proposed_end, status, response_note, responded_by, created_at "
...
def update_proposal(proposal_id, status, response_note="", responded_by=""):
    conn = get_conn()
    conn.execute(
        "UPDATE proposals SET status=?, response_note=?, responded_by=?, "
        "updated_at=datetime('now') WHERE id=?",
        (status, response_note, responded_by, proposal_id)
    )
    row = conn.execute(
        "SELECT meeting_code, proposer_name, proposed_date FROM proposals WHERE id=?",
        (proposal_id,)
    ).fetchone()
    if row:
        conn.execute(
            "INSERT INTO meeting_history (meeting_code, action, details) VALUES (?,?,?)",
            (row[0], "proposal_{}".format(status),
             "Proposal by {} for {}: {}".format(row[1], row[2], status))
        )
    conn.commit()
    conn.close()
