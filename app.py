...
TEXT = {
    "English": {
        ...
        "cancelled_status": "This meeting is cancelled and archived. New availability cannot be added.",
        ...
        "cancelled": "Meeting cancelled and moved to archive.",
        "archived": "Archived Meetings",
        "restore": "Restore Meeting",
        "restored": "Meeting restored.",
        "no_archived": "No archived meetings yet.",
        "status": "Status",
    ...
    "Türkçe": {
        ...
        "cancelled_status": "Bu toplantı iptal edilmiş ve arşivlenmiş. Yeni uygunluk eklenemez.",
        ...
        "cancel_meeting": "Toplantıyı İptal Et",
        "cancel_reason": "İptal Nedeni",
        "cancelled": "Toplantı iptal edildi ve arşive taşındı.",
        "archived": "Arşivlenen Toplantılar",
        "restore": "Toplantıyı Geri Al",
        "restored": "Toplantı geri alındı.",
        "no_archived": "Henüz arşivlenmiş toplantı yok.",
        "status": "Durum",
...
def is_cancelled(meeting):
    return meeting and len(meeting) >= 4 and meeting[3] == "cancelled"
...
# =========================================================
    ...
    if meeting:
        ...
        if is_cancelled(meeting):
            st.warning(t["cancelled_status"])
    elif meeting_code:
    ...
    if st.button(t["save"], use_container_width=True):
        meeting_check = get_meeting(meeting_code) if meeting_code else None

        if not meeting_code:
            st.warning(t["enter_code"])
        elif not meeting_check:
            st.warning(t["not_found"])
        elif is_cancelled(meeting_check):
            st.warning(t["cancelled_status"])
        elif not name:
...
# =========================================================
    ...
    guide("Cancel meetings without deleting data. Cancelled meetings are archived and can be restored later.")
    ...
    if meeting_code:
        ...
        if not meeting:
            ...
            st.info(
                f"Meeting: {meeting[1]}\n\n"
                f"Code: {meeting[0]}\n\n"
                f"{t['status']}: {meeting[3]}"
            )

            if is_cancelled(meeting):
                st.warning(t["cancelled_status"])
            else:
                if st.button(t["cancel_meeting"], use_container_width=True):
                    cancel_meeting(meeting_code, reason)
                    st.success(t["cancelled"])
                    st.rerun()

    card_end()

    st.subheader(t["archived"])
    ...
    if not archived:
        ...
        restore_code = st.selectbox(
            t["restore"],
            [row[0] for row in archived]
        )

        if st.button(t["restore"], use_container_width=True):
            restore_meeting(restore_code)
